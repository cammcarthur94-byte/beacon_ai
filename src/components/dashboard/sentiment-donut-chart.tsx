...
export function SentimentDonutChart(props: SentimentDonutChartProps) {
  ...
  const renderDonutContent = (isExpanded = false) => (
    <div className="grid grid-cols-12 gap-6 items-center w-full">
      {/* Donut Chart */}
      <div className={cn('col-span-12 sm:col-span-7 relative flex items-center justify-center', isExpanded ? 'h-[360px]' : 'h-[220px]')}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Pie
              ...
              cx="50%"
              cy="50%"
              innerRadius={isExpanded ? 90 : 62}
              outerRadius={isExpanded ? 145 : 96}
              paddingAngle={1.5}
              dataKey="value"
              cursor="pointer"
        ...
        {normalizedData.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex items-center gap-3 transition-all cursor-pointer text-left focus:outline-hidden group p-1.5 rounded-lg hover:bg-zinc-50',
                isSelected
                  ? 'scale-[1.02] bg-zinc-50'
                  : isAnySelected
  ...
  return (
    <>
      ...
      <ExpandableChartModal
        ...
        title="How AI Speaks About Your Brand"
        description={`Net tone score +${netScore}. Sentiment breakdown across search responses.`}
        exportFilename="brand-sentiment-tone"
        csvData={normalizedData.map((d) => ({
          Sentiment: d.name,
          Category: d.category,
          Percentage: `${d.value}%`,
        }))}
