...
export function PromptTrendChart({ data, queryText }: PromptTrendChartProps) {
  ...
  const renderTrendLines = (heightClass = 'h-[320px]') => (
  ...
  return (
    <>
      ...
      <ExpandableChartModal
        ...
        description={`Historical recommendation score trends across AI tools for "${queryText || 'search term'}".`}
        exportFilename="ai-visibility-trends"
        csvData={data}
        badge={badgeElement}
      >
        <div className="flex flex-col h-full w-full justify-between gap-3">
          ...
          <div className="flex-1 w-full min-h-[380px]">
            {renderTrendLines('h-[380px] sm:h-[420px]')}
          </div>
        </div>
      </ExpandableChartModal>
    </>
